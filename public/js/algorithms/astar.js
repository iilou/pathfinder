...
// function astar(map, M, N, start, end){
//     let sr = start[0];
//     let sc = start[1];
//     let fc = end[1];

//     let fmap = new Array(M).fill(-1).map(() => new Array(N).fill(-1));
//     fmap[sr][sc] = 0;
...
//     let open = new Array(M).fill(false).map(() => new Array(N).fill(false));
...
function astar(gridLayout, gridSize, sp, fp){
    isRunning = true;
    ...
    var fmap = new Array(M).fill(-1).map(() => new Array(N).fill(-1));
    ...
    function distRaw(p1, p2, q1, q2){
        // return Math.sqrt((p1-q1)*(p1-q1)+(p2-q2)*(p2-q2))  ;
        return (p1-q1)*(p1-q1)+(p2-q2)*(p2-q2)  ;
        // return 2*(Math.abs(p1-q1) + Math.abs(p2-q2));
        // return 3*(Math.abs(p1-q1) + Math.abs(p2-q2));
    }

    function stp(){
        cm = 100000;
        for(let i = 0; i < qr.length; i++){
            if(fmap[qr[i]][qc[i]] < cm){
                cm = fmap[qr[i]][qc[i]];
                ci = i;
            }
        }
        ...
        if(cr > 0){
            if(map[cr-1][cc] != -1 && !vis[cr-1][cc] && (dist[cr-1][cc] == -1 || dist[cr-1][cc] > dist[cr][cc] + map[cr-1][cc])){
                ...
                fmap[cr-1][cc] = dist[cr][cc] + map[cr-1][cc] + distRaw(cr-1, cc, fr, fc);
        ...
        if(cc > 0){
            if(map[cr][cc-1] != -1 && !vis[cr][cc-1] && (dist[cr][cc-1] == -1 || dist[cr][cc-1] > dist[cr][cc-1] + map[cr][cc-1])){
                ...
                fmap[cr][cc-1] = dist[cr][cc] + map[cr][cc-1] + distRaw(cr, cr-1, fr, fc);
        ...
        if(cc < N-1){
            if(map[cr][cc+1] != -1 && !vis[cr][cc+1] && (dist[cr][cc+1] == -1 || dist[cr][cc+1] > dist[cr][cc+1] + map[cr][cc+1])){
                ...
                dist[cr][cc+1] = dist[cr][cc] + map[cr][cc+1];
                lineList[cr][cc+1] = newLine(cr, cc, cr, cc+1);
                fmap[cr][cc+1] = dist[cr][cc] + map[cr][cc+1] + distRaw(cr, cc+1, fr, fc);
                updateCircle(cr, cc+1, true);
                updateCircle(cr, cc);
                pr[cr][cc+1] = cr;
                pc[cr][cc+1] = cc;
    ...
    function endSeq(){
        ...
        console.log(maxDist, dist[fr][fc]);
    }
}